...
#input_data_tmp_2 = input_data_tmp_1.loc[input_data_tmp_1['sensor'] == 6]
    
# no downsampling
input_data_tmp_2 = input_data_tmp_1.copy()

# reset index
input_data_tmp_2.reset_index(drop = True, inplace = True)
    
# standardize values by PowerTransformer
input_data_tmp_2.loc[:, ['value']] = PowerTransformer(method='yeo-johnson',).fit_transform\
(input_data_tmp_2.loc[:, ['value']])


input_data_tmp_4 = pd.DataFrame()
for sensor in pd.unique(input_data_tmp_2.loc[:, 'sensor']):
    input_data_tmp_3 = pd.DataFrame()
    input_data_tmp_3 = input_data_tmp_2.loc[input_data_tmp_2['sensor'] == sensor]
    
    # lagging and differcing 
    input_data_tmp_3['lag_0'] = input_data_tmp_3.loc[:, ['value']].shift()
    input_data_tmp_3['differences_0'] = input_data_tmp_3['lag_0'].diff()
    input_data_tmp_3['lag_2'] = input_data_tmp_3.loc[:, ['value']].shift(2)
    input_data_tmp_3['differences_2'] = input_data_tmp_3['lag_2'].diff()
    input_data_tmp_3['lag_4'] = input_data_tmp_3.loc[:, ['value']].shift(4)
    input_data_tmp_3['differences_4'] = input_data_tmp_3['lag_4'].diff()
    
    # drop rows with NaN
    input_data_tmp_3 = input_data_tmp_3.dropna()
    
    # append subsets to final DataFrame
    input_data_tmp_4 = input_data_tmp_4.append(input_data_tmp_3, 
                                             ignore_index = True, sort = False)
    
# build a out-of-bag set
max_date = input_data_tmp_4.loc[:, ['gregorian']].max()

# out-of-bag length
obb_length = max_date - horizon

# sort by timestamps
input_data_tmp = input_data_tmp_4.sort_values(by = 'gregorian')
...
if analysis == 'grid_search': 
    ...
    for i in range(n_iterations):
        ...
        param_grid = {'max_depth': [12], # 'max_depth' useful for controlling overfitting
                      ...
                      'colsample_bytree': [0.8], # 'colsample_bytree' useful for making training robust to noise; 
                      #'colsample_bytree': np.linspace(0.5, 1, 6),
                      'colsample_bylevel': [0.5], # 'colsample_bytree' useful for making training robust to noise; 
                      #'colsample_bylevel': np.linspace(0.5, 1, 6),
                      'colsample_bynode': [0.5], # 'colsample_bytree' useful for making training robust to noise; 
                      ...
                      'scale_pos_weight': [0.5], # 'scale_pos_weight' it re-balance data-set according to
...
if analysis == 'plotting_prediction':
    ...
    for sensor in pd.unique(input_data_tmp.loc[:, 'sensor']):
        ...
        sns.set(font_scale = 0.6, rc={'figure.figsize':(11.7,8.27)})
